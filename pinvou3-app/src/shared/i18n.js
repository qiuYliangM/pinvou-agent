...
const codexZh = {
  ...
  checkpoint:'检查点', checkpointBeforeTurn:turn=>`第 ${turn} 轮前`, checkpointLoading:'正在读取变更…',
  checkpointNoChanges:'该检查点之后没有文件变更',
  checkpointAdded:count=>`新增 ${count}`, checkpointModified:count=>`修改 ${count}`, checkpointDeleted:count=>`删除 ${count}`, checkpointOther:count=>`其他 ${count}`,
  checkpointRestore:'回滚到此', checkpointRestoreTitle:'把工作区文件回滚到该检查点；当前状态会先保存为新检查点，可再次回滚反悔', checkpointRestoring:'正在回滚…',
  checkpointConfirm:count=>count?`将撤销该检查点之后的 ${count} 个文件变更。当前状态会先保存为新的检查点，可以再次回滚反悔。确定回滚？`:'该检查点之后没有文件变更，回滚不会改变工作区。仍要继续吗？',
  checkpointFailed:'检查点操作失败',
  ...
  queueSend:'排队', queueSendTitle:'加入队列，当前轮次结束后自动发送',
  steerSend:'插队', steerSendTitle:'停止当前轮次，立即发送这条消息',
  stopTurn:'停止', stopTurnTitle:'停止当前轮次',
  queueMenuTitle:'发送方式', queueMenuClose:'关闭发送方式菜单',
  queueRemove:'从队列移除', queueSendFailed:'发送失败，队列已暂停',
  agentLog:'运行日志', agentLogTitle:'查看会话关键事件：轮次、工具调用、计划、检查点、错误',
  agentLogClose:'关闭运行日志', agentLogEmpty:'暂无事件；会话运行时的关键事件会按时间记录在这里',
  agentLogCapacityHint:'最多保留 200 条；重载后仅保留轮次历史',
  agentLogTurnStart:'轮次开始', agentLogTurnEnd:(status,duration)=>`轮次${status}${duration?` · 耗时 ${duration}`:''}`,
  agentLogPlanReady:count=>count?`方案待审批 · ${count} 步`:'方案待审批',
  agentLogPlanResolved:'方案已收口', agentLogPlanUpdate:count=>`计划更新 · ${count} 项`,
  agentLogCheckpointCreated:label=>`检查点创建${label?` · ${label}`:''}`,
  agentLogCheckpointRestored:'检查点回滚完成',
  agentLogPermissionRequested:'权限请求', agentLogPermissionResolved:summary=>`权限应答${summary?` · ${summary}`:''}`,
  agentLogBlocked:'已拦截',
  agentLogCopy:'复制此条', agentLogCopied:'已复制',
  agentLogExpand:'展开详情', agentLogCollapse:'收起详情',
  agentLogSkillHidden:'（技能已加载，内容不展示）',
};
const codexEn = {
  ...
  checkpoint:'Checkpoint', checkpointBeforeTurn:turn=>`before turn ${turn}`, checkpointLoading:'Loading changes…',
  checkpointNoChanges:'No file changes since this checkpoint',
  checkpointAdded:count=>`${count} added`, checkpointModified:count=>`${count} modified`, checkpointDeleted:count=>`${count} deleted`, checkpointOther:count=>`${count} other`,
  checkpointRestore:'Roll back to here', checkpointRestoreTitle:'Roll workspace files back to this checkpoint; the current state is saved as a new checkpoint first, so you can roll back again', checkpointRestoring:'Rolling back…',
  checkpointConfirm:count=>count?`This will undo ${count} file change(s) made after this checkpoint. The current state is saved as a new checkpoint first, so you can undo the rollback. Continue?`:'There are no file changes after this checkpoint, so rolling back will not change the workspace. Continue anyway?',
  ...
  // Run-time message queue/steer and session-level agent log (phase 2: session control & observability).
  queueSend:'Queue', queueSendTitle:'Queue this message; it sends automatically when the current turn ends',
  steerSend:'Steer', steerSendTitle:'Stop the current turn and send this message now',
  stopTurn:'Stop', stopTurnTitle:'Stop the current turn',
  queueMenuTitle:'Send options', queueMenuClose:'Close send options',
  queueRemove:'Remove from queue', queueSendFailed:'Send failed; queue paused',
  agentLog:'Run log', agentLogTitle:'View key session events: turns, tool calls, plans, checkpoints, errors',
  agentLogClose:'Close run log', agentLogEmpty:'No events yet. Key session events are recorded here in time order',
  agentLogCapacityHint:'Keeps up to 200 entries; only turn history survives reload',
  agentLogTurnStart:'Turn started', agentLogTurnEnd:(status,duration)=>`Turn ${String(status||'').toLowerCase()}${duration?` · took ${duration}`:''}`,
  agentLogPlanReady:count=>count?`Plan awaiting approval · ${count} steps`:'Plan awaiting approval',
  agentLogPlanResolved:'Plan settled', agentLogPlanUpdate:count=>`Plan update · ${count} items`,
  agentLogCheckpointCreated:label=>`Checkpoint created${label?` · ${label}`:''}`,
  agentLogCheckpointRestored:'Checkpoint restored',
  agentLogPermissionRequested:'Permission requested', agentLogPermissionResolved:summary=>`Permission answered${summary?` · ${summary}`:''}`,
  agentLogBlocked:'Blocked',
  agentLogCopy:'Copy entry', agentLogCopied:'Copied',
  agentLogExpand:'Expand details', agentLogCollapse:'Collapse details',
  agentLogSkillHidden:'(Skill loaded, content hidden)',
};
const codexJa = {
  ...
  checkpoint:'チェックポイント', checkpointBeforeTurn:turn=>`ターン ${turn} の前`, checkpointLoading:'変更を読み込み中…',
  checkpointNoChanges:'このチェックポイント以降のファイル変更はありません',
  checkpointAdded:count=>`追加 ${count}`, checkpointModified:count=>`変更 ${count}`, checkpointDeleted:count=>`削除 ${count}`, checkpointOther:count=>`その他 ${count}`,
  checkpointRestore:'ここまでロールバック', checkpointRestoreTitle:'ワークスペースのファイルをこのチェックポイントまでロールバックします。現在の状態は先に新しいチェックポイントとして保存されるため、やり直せます', checkpointRestoring:'ロールバック中…',
  checkpointConfirm:count=>count?`このチェックポイント以降の ${count} 件のファイル変更を取り消します。現在の状態は先に新しいチェックポイントとして保存されるため、ロールバックをやり直せます。続行しますか？`:'このチェックポイント以降のファイル変更はないため、ロールバックしてもワークスペースは変わりません。続行しますか？',
  checkpointFailed:'チェックポイント操作に失敗しました',
  ...
  // 実行中メッセージ queue/steer とセッション単位の agent log（フェーズ2・セッション制御と可観測性）。
  queueSend:'キュー', queueSendTitle:'キューに追加し、現在のターン終了後に自動送信します',
  steerSend:'割り込み', steerSendTitle:'現在のターンを停止して今すぐ送信します',
  stopTurn:'停止', stopTurnTitle:'現在のターンを停止します',
  queueMenuTitle:'送信方法', queueMenuClose:'送信方法メニューを閉じる',
  queueRemove:'キューから削除', queueSendFailed:'送信に失敗しました。キューは一時停止中です',
  agentLog:'実行ログ', agentLogTitle:'ターン、ツール呼び出し、プラン、チェックポイント、エラーなどの主要イベントを表示',
  agentLogClose:'実行ログを閉じる', agentLogEmpty:'イベントはまだありません。セッション実行中の主要イベントが時系列で記録されます',
  agentLogCapacityHint:'最大 200 件保持。再読み込み後はターン履歴のみ残ります',
  agentLogTurnStart:'ターン開始', agentLogTurnEnd:(status,duration)=>`ターン${status}${duration?` · ${duration}`:''}`,
  agentLogPlanReady:count=>count?`プラン承認待ち · ${count} ステップ`:'プラン承認待ち',
  agentLogPlanResolved:'プラン確定', agentLogPlanUpdate:count=>`プラン更新 · ${count} 件`,
  agentLogCheckpointCreated:label=>`チェックポイント作成${label?` · ${label}`:''}`,
  agentLogCheckpointRestored:'チェックポイントをロールバックしました',
  agentLogPermissionRequested:'権限リクエスト', agentLogPermissionResolved:summary=>`権限応答${summary?` · ${summary}`:''}`,
  agentLogBlocked:'ブロック済み',
  agentLogCopy:'この項目をコピー', agentLogCopied:'コピーしました',
  agentLogExpand:'詳細を展開', agentLogCollapse:'詳細を閉じる',
  agentLogSkillHidden:'（スキルを読み込みました。内容は非表示です）',
};
